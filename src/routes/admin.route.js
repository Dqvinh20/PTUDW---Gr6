...
router.get('/courses', async (req, res, next)=> {
    ...
    else if(cate == "all" && teacher != "all")
    {
        ...
        listCourse = await categoryModel.findCoursebyTeacherandCate(cate,teacher)
    }
...
.get(async (req, res, next)=> {
    ...
    if(categiries){
        for(let i=0;i<categiries.length;i++){
            ...
            const amount_course = await categoryModel.getAmountCourse(categiries[i].cat_id)
            const amount_children = await categoryModel.getAmountChild(categiries[i].cat_id)
...
.delete( async function (req, res) {
    const {slug} = req.params
    const category = await categoryModel.findbySlug(slug);
    const amount_course = await categoryModel.getAmountCourse(category.cat_id)
    const amount_children = await categoryModel.getAmountChild(category.cat_id)
...
router.post('/teachers', async function(req, res, next) {
    ...
    console.log(user)
    if (user) {
        return res.render("admin/addTeacherAccount", { layout: "adminLayout", errMessage : "Duplicate gmail" ,user});
    }
...
router.get('/teachers', async function(req, res, next) {
    ...
    if(list.length<6)
        nextpage = page;
    // const total = await categoryModel.findAll();
    res.render("admin/teacherAdmin", { layout: "adminLayout",users : list , isEmpty : list.length==0, prevpage:prevpage,
    nextpage: nextpage });
});
