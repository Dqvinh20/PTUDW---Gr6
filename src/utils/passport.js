...
/* LOCAL */
authPassport.use("local-login", new LocalStrategy({
        usernameField : 'email',
...
authPassport.use('email_verify', new MagicLinkStrategy.Strategy({
    ...
    console.log("SEND EMAIL", user, process.env['EMAIL'])
    ...
    const msg = {
        ...
        html: '<h3>Hello!</h3><p>Click the link below to finish verifying email of academyX account.</p><p><a href="' + link + '">Sign in</a></p>',
    };
    return sendgrid.send(msg).then(console.log).catch(console.error);
}, function verify(user) {
    return console.log("SEND EMAILx", user)
}))

/* FACEBOOK */
authPassport.use("fb-login", new FacebookStrategy({
    ...
    if (!cred) {
        return cb(null, false, { message : "Account don't exist.Please sign up first!"})
    } else {
...
authPassport.use("fb-signup", new FacebookStrategy({
    clientID: process.env.FACEBOOK_CLIENT_ID,
    clientSecret: process.env.FACEBOOK_CLIENT_SECRET,
    ...
    if (!cred) {
        ...
        return cb(null, false, { message: "Account is already exist!!!"});
    }
}));

/* GOOGLE */
authPassport.use("gg-login", new GoogleStrategy.Strategy({
...
authPassport.use("gg-signup", new GoogleStrategy.Strategy({
    clientID: process.env.GOOGLE_CLIENT_ID,
    clientSecret: process.env.GOOGLE_CLIENT_SECRET,
...
authPassport.serializeUser(function (user, cb) {
    process.nextTick(async function () {
        if (user.photo)
            await downloadImg(user.photo, './public/images/user_avatar.png')
        cb(null, {
            id: user.id,
            email: user.email,
